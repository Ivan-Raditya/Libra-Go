...
class ShoppingDetailScreen extends StatefulWidget {
  ...
  final int? month;
  final int? year;
  const ShoppingDetailScreen({super.key, this.tripId, this.month, this.year});
  ...
  Future<void> _loadData() async {
    try {
      ...
      if (widget.month != null && widget.year != null) {
        expenses = await _supabase.getMonthlyExpensesByCategory('shopping', widget.month!, widget.year!);
      } else {
        ...
        expenses = await _supabase.getExpensesByCategory('shopping', tripId: _activeTripId);
      }
  ...
  Future<void> _deleteAllHistory() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Apakah Anda yakin ingin menghapus semua riwayat pengeluaran belanja untuk perjalanan ini?'),
        actions: [
    ...
    if (confirm == true) {
      ...
      try {
        await _supabase.deleteExpensesByCategory('shopping', tripId: _activeTripId);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Semua riwayat belanja telah dihapus')));
          _loadData();
        }
      } catch (e) {
        if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Detail Belanja', style: TextStyle(color: Color(0xFF0D1B2A), fontWeight: FontWeight.bold, fontSize: 16)),
        actions: [
                ...
                children: [
                  Container(
                    ...
                    decoration: const BoxDecoration(
                      gradient: LinearGradient(colors: [Color(0xFFE8F0FE), Color(0xFFF7F9FB)], begin: Alignment.topCenter, end: Alignment.bottomCenter),
                    ),
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        Expanded(child: _buildStatCard(icon: Icons.shopping_bag, title: 'Total Items', value: '${_expenses.length}', subtitle: 'this trip')),
                  ...
                  if (_expenses.isEmpty)
                    Padding(
                      ...
                      child: Container(
                        ...
                        child: Column(children: const [Icon(Icons.shopping_bag_outlined, color: Color(0xFFE8ECEF), size: 40), SizedBox(height: 8), Text('Belum ada transaksi belanja', style: TextStyle(color: Color(0xFF596273)))]),
                      ),
                    )
                  else
                    Padding(
                        ...
                        children: _expenses.map((expense) {
                          ...
                          return Padding(
                            ...
                            child: GestureDetector(
                              ...
                              child: _buildTransactionCard(Icons.shopping_bag_outlined, expense['name'] ?? '-', dateStr, _formatRupiah((expense['amount'] as num).toInt())),
                            ),
                          );
                        }).toList(),
                      ),
  ...
  Widget _buildTransactionCard(IconData icon, String title, String subtitle, String amount) {
    return Container(
      ...
      child: Row(children: [
        ...
        Text(amount, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Color(0xFF0D1B2A))),
      ]),
    );
  }
}
