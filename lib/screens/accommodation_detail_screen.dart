...
class AccommodationDetailScreen extends StatefulWidget {
  ...
  final int? month;
  final int? year;
  const AccommodationDetailScreen({super.key, this.tripId, this.month, this.year});
  ...
  Future<void> _loadData() async {
    try {
      ...
      if (widget.month != null && widget.year != null) {
        expenses = await _supabase.getMonthlyExpensesByCategory('accommodation', widget.month!, widget.year!);
      } else {
        ...
        expenses = await _supabase.getExpensesByCategory('accommodation', tripId: _activeTripId);
      }
  ...
  Future<void> _deleteAllHistory() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Apakah Anda yakin ingin menghapus semua riwayat pengeluaran akomodasi untuk perjalanan ini?'),
        actions: [
    ...
    if (confirm == true) {
      ...
      try {
        await _supabase.deleteExpensesByCategory('accommodation', tripId: _activeTripId);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Semua riwayat akomodasi telah dihapus')));
          _loadData();
        }
      } catch (e) {
        if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Detail Akomodasi', style: TextStyle(color: Color(0xFF0D1B2A), fontWeight: FontWeight.bold, fontSize: 16)),
        actions: [
                ...
                children: [
                  Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('TOTAL AKOMODASI', style: TextStyle(color: Color(0xFF596273), fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1.0)),
                  ...
                  const Padding(
                    ...
                    child: Text('Riwayat Akomodasi', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF0D1B2A))),
                  ),
                  const SizedBox(height: 16),
                  ...
                  if (_expenses.isEmpty)
                    Padding(
                      ...
                      child: Container(
                        ...
                        child: Column(children: const [Icon(Icons.bed, color: Color(0xFFE8ECEF), size: 40), SizedBox(height: 8), Text('Belum ada pengeluaran akomodasi', style: TextStyle(color: Color(0xFF596273)))]),
                      ),
                    )
                  else
                      ...
                      return Padding(
                        ...
                        child: GestureDetector(
                          ...
                          child: Container(
                            ...
                            child: Row(children: [
                              Container(padding: const EdgeInsets.all(12), decoration: const BoxDecoration(color: Color(0xFF0D1B2A), shape: BoxShape.circle), child: const Icon(Icons.bed, color: Colors.white, size: 20)),
